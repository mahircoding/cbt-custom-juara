                        ...
                        <table class="table mb-0" style="font-size:10pt;">
                            ...
                            <tbody>
                                <tr v-for="(lesson, index) in lessons.data" :key="index">
                                    ...
                                    <td>
                                        <div class="btn-group">
                                            <button type="button" :class="{'btn': true, 'btn-sm': true, 'btn-danger': lesson.development_status === 'development', 'btn-primary': lesson.development_status === 'production'}">{{ capitalizeWords(lesson.development_status) }}</button>
                                            <button type="button" :class="{'btn': true, 'btn-sm': true, 'btn-danger': lesson.development_status === 'development', 'btn-primary': lesson.development_status === 'production', 'split-bg-warning': lesson.development_status === 'inprogress', 'split-bg-danger': lesson.development_status === 'development', 'split-bg-primary': lesson.development_status === 'production'}" class="dropdown-toggle dropdown-toggle-split" data-bs-toggle="dropdown" aria-expanded="false">	<span class="visually-hidden">Toggle Dropdown</span></button>
                                            <ul class="dropdown-menu">
                                                <li v-if="lesson.development_status !== 'production'"><Link class="dropdown-item" :href="`/admin/lessons/${lesson.id}/change-status?development_status=production`">Production</Link></li>
                                                <li v-if="lesson.development_status !== 'development'"><Link class="dropdown-item" :href="`/admin/lessons/${lesson.id}/change-status?development_status=development`">Development</Link></li>
                                            </ul>
                                        </div>
                                    </td>
                                    <td>
