            ...
            <div class="card border-top border-0 border-3 border-primary">
                <div class="card-body">
                    <div class="d-lg-flex align-items-center mb-4 gap-3">
                        <Link :href="`/admin/value-category-groups/${valueCategoryGroup.id}/value-categories/${valueCategory.id}/detail-value-categories/create`" class="btn btn-primary btn-sm mt-2 mt-lg-0"><i class="bx bxs-plus-square"></i> Tambah Kategori Penilaian</Link>
                        ...
                        <table class="table mb-0">
                            ...
                            <tbody>
                                <tr v-for="(detailValueCategory, index) in detailValueCategories.data" :key="index">
                                    ...
                                    <td>{{ detailValueCategory.min_grade ?? 'Nilai Minimal Tak Hingga' }}</td>
                                    <td>{{ detailValueCategory.max_grade ?? 'Nilai Maksimal Tak Hingga' }}</td>
                                    <td>{{ detailValueCategory.category_grade }}</td>
                                    <td>{{ detailValueCategory.final_grade }}</td>
                                    <td>
                                        <div class="d-flex order-actions">
                                            <Link :href="`/admin/value-category-groups/${valueCategoryGroup.id}/value-categories/${valueCategory.id}/detail-value-categories/${detailValueCategory.id}/edit`" class="ms-1"><i class='bx bxs-edit'></i></Link>
                                            <a href="#" @click.prevent="destroy(detailValueCategory.id)" class="ms-1"><i class="bx bxs-trash"></i></a>
                                        </div>
                                    </td>
                                </tr>
                                <tr v-if="!detailValueCategories.data.length">
            ...
            const destroy = (id) => {
                ...
                .then((result) => {
                    if (result.isConfirmed) {
                        ...
                        Inertia.delete(`/admin/value-category-groups/${props.valueCategoryGroup.id}/value-categories/${props.valueCategory.id}/detail-value-categories/${id}`);
