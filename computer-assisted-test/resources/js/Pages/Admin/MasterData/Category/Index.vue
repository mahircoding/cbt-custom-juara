                        ...
                        <table class="table mb-0" style="font-size:10pt;">
                            ...
                            <tbody>
                                <tr v-for="(category, index) in categories.data" :key="index">
                                    ...
                                    <td>
                                        <div class="btn-group">
                                            <button type="button" :class="{'btn': true, 'btn-sm': true, 'btn-danger': category.development_status === 'development', 'btn-primary': category.development_status === 'production'}">{{ capitalizeWords(category.development_status) }}</button>
                                            <button type="button" :class="{'btn': true, 'btn-sm': true, 'btn-danger': category.development_status === 'development', 'btn-primary': category.development_status === 'production', 'split-bg-warning': category.development_status === 'inprogress', 'split-bg-danger': category.development_status === 'development', 'split-bg-primary': category.development_status === 'production'}" class="dropdown-toggle dropdown-toggle-split" data-bs-toggle="dropdown" aria-expanded="false">	<span class="visually-hidden">Toggle Dropdown</span></button>
                                            <ul class="dropdown-menu">
                                                <li v-if="category.development_status !== 'production'"><Link class="dropdown-item" :href="`/admin/categories/${category.id}/change-status?development_status=production`">Production</Link></li>
                                                <li v-if="category.development_status !== 'development'"><Link class="dropdown-item" :href="`/admin/categories/${category.id}/change-status?development_status=development`">Development</Link></li>
                                            </ul>
                                        </div>
                                    </td>
                                    <td>
                                ...
                                <tr>
                                    <td align="center" colspan="6" v-if="!categories.data.length">Data Tidak Tersedia</td>
                                </tr>
                            </tbody>
                        </table>
                    </div>
