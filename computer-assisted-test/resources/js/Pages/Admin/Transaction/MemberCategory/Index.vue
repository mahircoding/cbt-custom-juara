            ...
            <div class="card border-top border-0 border-3 border-primary">
                <div class="card-body">
                    <div class="d-lg-flex align-items-center mb-4 gap-3">
                        ...
                        <div class="ms-auto">
                            <Link href="/admin/member-categories/create" class="btn btn-primary btn-sm mt-2 mt-lg-0"><i class="bx bxs-plus-square"></i> Tambah Kategori Member</Link>
                        </div>
                    </div>
                        ...
                        <table class="table mb-0" style="font-size:10pt;">
                            ...
                            <tbody>
                                <tr v-for="(memberCategory, index) in memberCategories.data" :key="index">
                                    <td>{{ ++index + (memberCategories.current_page - 1) * memberCategories.per_page }}</td>
                                    <td>{{ memberCategory.name }}</td>
                                    <td>{{ memberCategory.description ?? '-' }}</td>
                                    <td>
                                        <span v-if="memberCategory.status == 1" class="badge bg-success">Active</span>
                                        <span v-else class="badge bg-danger">Inactive</span>
                                    </td>
                                    <td>
                                        <div class="d-flex order-actions">
                                            <Link :href="`/admin/member-categories/${memberCategory.id}/edit`" class="ms-1"><i class='bx bxs-edit'></i></Link>
                                            <a href="#" @click.prevent="destroy(memberCategory.id)" class="ms-1"><i class="bx bxs-trash"></i></a>
                                        </div>
                                    </td>
                                </tr>
                                <tr>
                                    <td align="center" colspan="7" v-if="!memberCategories.data.length">Data Tidak Tersedia</td>
                                </tr>
                            </tbody>
                        </table>
                    </div>
