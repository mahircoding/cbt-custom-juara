                ...
                <div class="col-xs-12 col-sm-6 col-md-6 col-lg-3" v-for="(exam, index) in examGroups.data" :key="index">
                    <div class="card border-bottom border-3 border-0">       
                        ...
                        <div class="card-header">
                            ...
                            <p class="card-text">Kerjakan Tryout Sesuai Perintah Yang Ada Dalam Informasi.</p>
                        </div>
                        <div class="card-body" v-if="$page.props.auth.user.member_type == 2">  
                            <div v-if="($page.props.setting.purchase_type == 1 || $page.props.setting.purchase_type == 3)"  style="min-height: 50px;">
                                <div v-if="exam.price_type == 2">
                                    ...
                                    <div v-else>
                                        <h6 class="text-dark text-center"><sup><s>Rp.{{ formatPrice(exam.price_before_discount) }}</s><span class="badge bg-danger mx-1">{{ formatPrice((exam.price_before_discount - exam.price_after_discount) / exam.price_before_discount * 100)}}% </span></sup><br>Rp.{{ formatPrice(exam.price_after_discount) }}</h6>
                                    </div>     
                                </div>      
                                <div v-else>
                                    <h6 class="text-dark text-center">Gratis</h6>
                                </div>                                         
                            </div>
                        ...
                        <div class="card-footer">
                            <div v-if="exam.exam_status == 'active'">
                                <div v-if="$page.props.auth.user.member_type == 2 & exam.price_type == 2">
                                    <div v-if="$page.props.setting.purchase_type == 1">
                                        <div v-if="exam.transaction.length > 0">
                                            <Link :href="`/user/exam-groups/${lessonCategory.category_id}/lesson-categories/${lessonCategory.id}/exams/${exam.id}`" class="btn btn-primary btn-sm w-100">Kerjakan Tryout</Link>
                                        </div>
                                        <div v-else>
                                            <button class="btn btn-outline-primary btn-sm w-100" @click="buyExam(exam.id)">Beli Tryout</button>         
                                        </div>
                                    </div>
                                    ...
                                    <div v-if="$page.props.setting.purchase_type == 3">
                                        <div v-if="exam.transaction.length > 0 && checkMemberCategories(exam.member_categories) == true">
                                            <Link :href="`/user/exam-groups/${lessonCategory.category_id}/lesson-categories/${lessonCategory.id}/exams/${exam.id}`" class="btn btn-primary btn-sm w-100 mb-2">Kerjakan Tryout</Link>
                                        </div>
                                        ...
                                        <div v-if="exam.transaction.length == 0">
                                            <button class="btn btn-outline-primary btn-sm w-100 mb-2" @click="buyExam(exam.id)">Beli Tryout</button>       
                                        </div>
    ...
    export default {
        ...
        setup(props) {
            ...
            const buyExam = (examId) => {
                Swal.fire({
                    title: 'yakin akan membeli Tryout ?',
                    text: "Pastikan Saldo Anda Cukup Untuk Transaksi",
                    icon: 'warning',
                    showCancelButton: true,
                    confirmButtonColor: '#3085d6',
                    cancelButtonColor: '#d33',
                    confirmButtonText: 'Ya, Beli Tryout',
                    cancelButtonText: 'Tidak'
                })
                .then((result) => {
                    if (result.isConfirmed) {
