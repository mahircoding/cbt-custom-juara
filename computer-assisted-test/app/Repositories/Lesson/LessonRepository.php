...
class LessonRepository extends BaseRepository implements LessonInterface
{
    ...
    public function getAllPaginatedWithParams($params, $limit = 10)
    {
        ...
        if(isset($params->search) && !empty($params->search)) $lessons->where('lessons.name', 'like', '%' . $params->search . '%');

        $lessons = $lessons->join('lesson_categories', 'lesson_categories.id', '=', 'lessons.lesson_category_id')
            ->join('categories', 'categories.id', '=', 'lesson_categories.category_id')
    ...
    {
        return $this->model->with('lessonCategory', 'lessonCategory.category')->where(['lesson_category_id' => $lessonCategoryId, 'development_status' => 'production'])->orderBy('order', 'ASC')->get();
    }
