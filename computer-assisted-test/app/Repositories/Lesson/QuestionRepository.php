...
class QuestionRepository extends BaseRepository implements QuestionInterface
{
    ...
    public function create($attributes)
    {
        ...
        if (!empty($input['answer']) && is_array($input['answer'])) {
            // Urutkan array berdasarkan ID
            usort($input['answer'], function($a, $b) {
                return $a['id'] <=> $b['id'];
            });
            // Ambil ID setelah diurutkan
            $answer = array_map(function($answer) {
                return $answer['id'];
            }, $input['answer']);
        }

        $input['answer'] = $answer;
        $input['total_answer_limit'] = $answer == null ? 1 : count($answer);
