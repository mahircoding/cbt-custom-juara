...
class HomeController extends Controller
{
    ...
    public function redirectExam($type, $code)
    {
        $typeModel = [
            'latihansoal' => Exam::class,
            'tryout'      => ExamGroup::class,
        ];
    
        if (!array_key_exists($type, $typeModel)) {
            return response('Tipe tidak dikenali', 404);
        }
    
        $model = $typeModel[$type];
    
        $data = $model::where('short_code_link', $code)->first();
    
        if ($data) {
            if($type == 'tryout') {
                return redirect()->route('user.exam_group.show', [
                    'id' => $data->category_id,
                    'lessonCategoryId' => $data->lesson_category_id,
                    'examGroupId' => $data->id,
                ]);
            }
            else {
                return redirect()->route('user.categories.lesson-categories.lessons.exams.show', [
                    ...
                    'lesson_category' => $data->lesson_category_id,
